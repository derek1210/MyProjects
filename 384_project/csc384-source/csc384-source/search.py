...
def _fval_function(state, weight):
...
class sNode:
    ...
    def __init__(self, state, hval, fval_function, weight):
        self.state = state
        self.hval = hval
    ...
    def __lt__(self, other):
        ...
        if sNode.lt_type == _C:  
            return self.fval_function(self, self.weight) <  other.fval_function(other, other.weight)          
...
class SearchEngine:
    ...
    def search(self, initState, goal_fn, heur_fn = _zero_hfn, timebound = 10, fval_function = _fval_function, weight = 0, costbound = 10000000000):
        ...
        goal_node = []
        node = sNode(initState, heur_fn(initState), fval_function, weight)      
